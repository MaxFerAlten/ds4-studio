 ...
 * @param {object} options.tokens - { serviceToken, modelGatewayToken }
...
export function createAgnoProcessManager({
  ...
  const buildEnv = () => ({
    ...
    DS4_AGNO_DS4_STUDIO_BASE_URL: `http://${controlHost}:${controlPort}`,
  ...
  const healthCheck = async () => {
    const url = `http://${ag.host}:${ag.port}/ds4/health`;
    try {
